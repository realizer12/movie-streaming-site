...
<nav class="navbar navbar-inverse" style="color: white; height:30px; background:white;">
  <ul class="nav navbar-nav" style="color:black; height:30px; background:white;">
    ...
    <li class="w3-button "onclick="location.href='onairmovie.php'"  style="margin-left: 100px; background-color:rgb(195, 196, 194); height:48px;font-size: 20px">
      현재 상영작 예매하기
    </li>
...
<!--메인화면의 중앙 부분입니다.  -->
<?php
include ("simple_html_dom.php");//따로 파일을 깔아서 hdtocs폴더 안에 넣어놈.

$ch= curl_init();//curl을 넣어서 기능을 실현시키기위해 변수에 담았다.
curl_setopt($ch,CURLOPT_URL,'http://www.cgv.co.kr/movies/');
curl_setopt($ch,CURLOPT_FOLLOWLOCATION,1);
curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
$response=curl_exec($ch);//curl을 실행시킨다.   cgv영화를 가져온다.
curl_close($ch);//curl닫아줌


//echo $response;
$html=new simple_html_dom();//다운 받은 심플 해티엘에돔 파일을선언한다.
$html->load($response);//아까  가져온 cgv파일을로드한다.
//foreach ($html->find('a[href]') as $link)
//echo $link->plaintext.'</br>'


//
foreach ($html->find('img[onerror="errorImage(this)"]') as $item){//내 입맛에 맞게 바
  //$item= $link->find('img[onerror="errorImage(this)"]',0);
   $articles[] = $item;// 이렇게 하면 사진을 가지고올수있음>

}

foreach ($html->find('strong[class="title"]') as $title){//내 입맛에 맞게 바
  // $item= $link->find('img[onerror="errorImage(this)"]',0);
   $movietitle[] = $title;// 이렇게 하면 사진을 가지고올수있음>

}
foreach ($html->find('a[class="link-reservation"]') as $ticket){//내 입맛에 맞게 바
  // $item= $link->find('img[onerror="errorImage(this)"]',0);
   $movieticket[] = $ticket->href;// 이렇게 하면 사진을 가지고올수있음>

}
?>

<div style="margin-left:50px; margin-top:50px;">
  <font style="font-size:60px;">현재 상영작 예매하기 </font><br>
  <font style="font-size:30px; margin-left:10px;"> 영화관예매로 연결시켜주는 서비스입니다!</font>
</div>
<div style="margin-top:50px;">


<div  style="margin-left:60px;margin-top:30px;">
<font style="font-size:30px;">아래는</font>  <font style="font-size:40px; color:red;">CGV</font>  <font style="font-size:30px;">에서 상영중인 영화 상위 </font> <font style="font-size:40px; color:red;">7</font><font style="font-size:30px;">개 입니다.</font>
</div>
<div id="cgvcontainer" style=" background-color:rgba(23, 23, 19, 0.96); width:2190px; margin-top:5px; margin-left: 50px;height: 430px;border: 3px solid #1d0606;">
<?php for($i=0; $i<7; $i++){ ?>
<div style="background-color:rgb(237, 154, 12); margin-left:60px; margin-top: 20px; float: left;width:191px; border: 3px solid #1d0606;">
<font style="color:black;margin-left: 5px;font-size:30px;"><?php echo $i+1; ?>위</font>
<?php echo $articles[$i];?></br>
<font style="font-size:17px; color:black; margin-left:5px;"><?php echo $movietitle[$i]; ?></font>

<div style="margin-top:2px;">
  <a id="getticket" href="http://www.cgv.co.kr/<?php echo $movieticket[$i]; ?>" onclick="window.open(this.href, '_blank', 'width=1000,height=700,toolbars=no,scrollbars=no'); return false;" >예매하기</a>
</div>
</div>
<?php } ?>
</div>
