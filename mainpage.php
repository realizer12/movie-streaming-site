...

{
...

 {
  ...
  var getCookie1=getCookie('ad');
  if(getCookie1==null){
  window.open("adpopup.php",  "popupNo1", "width=300, height=390");
}else{
...
<nav class="navbar navbar-inverse" style="color: white; height:30px; background:white;">
  <ul class="nav navbar-nav" style="color:black; height:30px; background:white;">
    ...
    <li class="w3-button " onclick="location.href='mainpage.php'"style="margin-left: 200px;background-color:rgb(195, 196, 194);height:48px;font-size: 20px">
    메인화면
    </li>
...
$conn = mysqli_connect($mysql_host, $mysql_user, $mysql_password,$mysql_db);
$sqlformovie = "SELECT * FROM movie ORDER BY movienumber DESC  LIMIT 5";//업로드 순//w전체;

// $sql = "select count(*) from test3 where id='$id'";
$Resultformovie = mysqli_query($conn,$sqlformovie);



 ?>
<div id="myCarousel" class="carousel slide" data-ride="carousel" style="margin-left: 20px; margin-top:30px; background-color: rgba(29, 29, 26, 0.89);width:1850px;border: 1.5px solid #1d0606;   height: 492px;">
    <!-- Indicators -->
    ...
    <div class="carousel-inner"  role="listbox" >
      <div class="item active" >
        ...
        <br>
        <br>
              <font style="font-size:70px; color:white; margin-left:530px; ">금주의 최신 업로드된  영화<font style="color:red; font-size:80px;">5</font>개</font>
      </div>
<?php while(  $rowsformovie = mysqli_fetch_array($Resultformovie)){ ?>
      <div class="item ">

        <img src="<?php echo $rowsformovie[posterpath]; ?>" onclick="location.href='watchmovie.php'"alt="hobby2"  style=" cursor: pointer; margin-left:745px; margin-top:20px; width:350px; height:350px; object-fit: contain"/>




        <a href="watchmovie.php"style="color:white;margin-left:825px; margin-top:5px; font-size:30px;">제목:<?php echo $rowsformovie[movietitle];  ?></a>
...
$conn = mysqli_connect($mysql_host, $mysql_user, $mysql_password,$mysql_db);
$sqlformovieblog = "SELECT * FROM movieblog ORDER BY idnum DESC  LIMIT 10";//업로드 순//w전체;
// $sql = "select count(*) from test3 where id='$id'";
$Resultformovieblog = mysqli_query($conn,$sqlformovieblog);
//영화 블로그  글을 가지고 온다.
 ?>
<div style="margin-left: 20px; margin-top:30px; width:910px;border: 1.5px solid #1d0606;   height: 412px;">
<!--이쪽에는 영화블로그 최신 글이 들어간다.  -->
<div style="border-bottom: 1.5px solid #1d0606;background-color:rgba(124, 125, 124, 0.76); " >
<font style="font-size:30px; margin-left:50px; ">영화 후기  최신글<font style="color:red;" > 10</font> 개</font>
</div>

<div style="margin-left:50px; margin-top:30px;">
  <?php while ($rowsformovieblog = mysqli_fetch_array($Resultformovieblog)) { ?>
  ● <a style="font-size:20px;"href="readmovieblog.php?number=<?php echo $rowsformovieblog[idnum]; ?>"><?php echo $rowsformovieblog[title]; ?></a><br>
<?php }?>
</div>
</div>
<?php
...
$conn = mysqli_connect($mysql_host, $mysql_user, $mysql_password,$mysql_db);
$sqlformovieblog1 = "SELECT * FROM movieblog ORDER BY view DESC  LIMIT 10";//업로드 순//w전체;
// $sql = "select count(*) from test3 where id='$id'";
$Resultformovieblog1 = mysqli_query($conn,$sqlformovieblog1);
//영화 블로그  글을 가지고 온다.
 ?>
<div style="margin-left: 20px; margin-top:30px; float: right;width:910px;border: 1.5px solid #1d0606; margin-left:960px;  float: left;margin-top: -412px;height: 412px;">
<!--이쪽에는 영화 블로그 가장 조회수가 많은  내용이 들어간다.  -->
<div style="border-bottom: 1.5px solid #1d0606;background-color:rgba(124, 125, 124, 0.76); " >
<font style="font-size:30px; margin-left:50px; ">영화 후기 인기글<font style="color:red;" > 10</font> 개 <font style=" font-size:20px;" >(조회순)</font> </font>
</div>
<div style="margin-left:50px; margin-top:30px;">
  <?php while ($rowsformovieblog1 = mysqli_fetch_array($Resultformovieblog1)) { ?>
  ● <a style="font-size:20px;"href="readmovieblog.php?number=<?php echo $rowsformovieblog1[idnum]; ?>"><?php echo $rowsformovieblog1[title]; ?></a><br>
<?php }?>
</div>
