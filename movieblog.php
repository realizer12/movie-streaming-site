...
<nav class="navbar navbar-inverse" style="color: white; height:30px; background:white;">
  <ul class="nav navbar-nav" style="color:black; height:30px; background:white;">
    ...
    <li class="w3-button " onclick="location.href='movieblog.php'" style="margin-left: 100px; background-color:rgb(195, 196, 194);height:48px;font-size: 20px">
      영화 후기 블로그
    </li>
...
if($blogtype==1){
 ...
 <p style="margin-right: 5px;margin-top: 10px;font-size:30px;">영화를 보고 느낀 후기를 실시간 방송으 표현해보세요!</p>
 <!--게시판의 글을 쓰기위해  글을 쓰는 페이지로 넘어간다.  -->
 <div style="margin-top:0px; font-size:30px; ">
 <a id="blogtype0" class="selectblogtype"href="movieblog.php?blogtype=0">글로 후기남기기</a>
 <!-- <a id="blogtype1" class="selectblogtype" href="movieblog.php?blogtype=1">실시간 방송으로 후기 남기기</a> --> -->
 </div>
 <button type="button" id="wirte-btn-notice-board" name="wirte-btn-notice-board" onclick="'">방송 방 만들기</button>

 </div>
<script>$("#blogtype1").css('background-color','rgba(132, 138, 136, 0.83)');</script>
...
<p style="margin-right: 5px;margin-top: 10px;font-size:30px;">영화를 보고 느낀 후기를 공유해보세요!</p>
<!--게시판의 글을 쓰기위해  글을 쓰는 페이지로 넘어간다.  -->
<div style="margin-top:0px; font-size:30px; ">
...
<button type="button" id="wirte-btn-notice-board" name="wirte-btn-notice-board" onclick="location.href='writemovieblog.php'">후기 쓰러가기</button>

</div>
<script>$("#blogtype0").css('background-color','rgba(132, 138, 136, 0.83)');</script>
<div id="notice-board-main">
  ...
  $query = "SELECT * FROM movieblog ORDER BY thread DESC LIMIT $no,$page_size";//NO 가 들어간 이유는 페이지 번호 즉 각 페이지 마다 제한되어서 나타는 리스트의 크기를 보여주기 위함이다
  //EX  NO=2  $page_size=8일때  2페이지의  보이는 리스트는  8개이다. 이다.
  $resultt = mysqli_query($conn,$query);
  $resulet_sql="SELECT*FROM movieblog";
  $total_row = mysqli_num_rows(mysqli_query($conn,$resulet_sql));
  }else{
   if($type==0){
   //이경우에는 제목으로 검색한 경우이다.
   $query = "SELECT * FROM movieblog where title like'%$searchthing%' ORDER BY thread DESC LIMIT $no,$page_size";
   $resultt = mysqli_query($conn,$query);
   $resulet_sql="SELECT*FROM movieblog where title like'%$searchthing%'";
   $total_row = mysqli_num_rows(mysqli_query($conn,$resulet_sql));

   }else if($type==1){
   //이경우에는 작성자로 검색한 경우이다.
   $query = "SELECT * FROM movieblog where id like'%$searchthing%' ORDER BY thread DESC LIMIT $no,$page_size";
   $resultt = mysqli_query($conn,$query);
   $resulet_sql="SELECT*FROM movieblog where id like'%$searchthing%'";
   $total_row = mysqli_num_rows(mysqli_query($conn,$resulet_sql));
   }
      ...
      while(  $row=mysqli_fetch_array($resultt))
       {
         ...
         //아이디가 존재하는경우
         $total_count1="SELECT id FROM blogcomment WHERE articlenum = $articlenumforcountcomment ";//현재 페이지의  댓글의 개수 가 몇개인지알아보기위한 쿼리 문이다.
        ...
        <a style="cursor:pointer"onclick="location.href='readmovieblog.php?number=<?php echo $row[idnum] ?>&pagenum=<?php echo $no ?>&opt=<?php echo $type ?>&searchfromcontent=<?php echo $searchthing ?>'">
        <?php if($row[checkimg]==1){ //컨테츠안에 이미지가 있을때이다.?>
        <img style="width:15px; margin-right:10px;   border:1px solid #000;" src="webimage/imagecheck.jpg"><?php echo $row[title] ?> <font style="margin-left:1px; color:rgb(242, 65, 53)"><?php if($count1>0) {?>(<?php echo $count1 ?>)<?php } ?></font>
        <?php }else{ //컨텐츠 안에 이미지가 없을때?>
        <?php echo $row[title] ?> <font style="margin-left:1px; color:rgb(242, 65, 53)"><?php if($count1>0) {?>(<?php echo $count1 ?>)<?php } ?></font>
        <?php } ?>
        </a>                                   <!--위쪽 부분이 댓글 관련 판단하고 띄우는 란이다,  -->
           </td>
  ...
  <script type="text/javascript">
    function serachnotice33() {
      ...
      if( searchform123 == ""){
        ...
        <form name="serachnoticeform" onsubmit="return serachnotice33();" action="movieblog.php?datatype=<?php echo $_GET['opt'] ?>&data=<?php echo $_GET['searchfromcontent'] ?>" method="get" >
