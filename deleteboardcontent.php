...
$total_count="SELECT id FROM comment WHERE articlenum = $ddddd ";//현재 페이지의  댓글의 개수 가 몇개인지알아보기위한 쿼리 문이다.
...
$idment="작성자미상";
if($count>0){//게시글의  댓글수가 0이상일때 
  $sqlupdate1="UPDATE board SET title ='$titlement', id='$idment' where idnum ='$ddddd'";//부모글의  seq가 1로 바뀌면  나중에 삭제해도 삭된 글이라고 뜸.
  $Result1=mysqli_query($conn,$sqlupdate1);
}else{
$sql1 = "DELETE FROM board WHERE idnum ='$ddddd'";
