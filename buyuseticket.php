...
<?php }else{ ?>
  <span style="font-size:20px; margin-left:20px;">영화 이용권: </span><a style="font-size:20px; color:blue; " > <?php echo date("Y-m-d",$rows[usedate]); ?>까지 이용가능 </a>
<?php } ?>
  </br>
...
var five= document.getElementById('ex_rd').checked;
var ten=document.getElementById('ex_rd1').checked;
...
<div style="  border: 2px solid #1d0606; width:1200px; margin-left:350px; margin-top:40px;height:600px;">
<font style="font-size:50px; margin-left:10px;">사이트 이용권 구매하기</font><br>
<font style="font-size:30px; margin-left:10px;">지난 영화를 보기 위해서는 다음 이용권을 구매하세요!</font>

<div class="checks" style="margin-left:400px; margin-top:120px;">
  <input type="radio" style="width:30px; height:30px; margin-bottom:100px;"id="ex_rd" name="ex_rds">
<label for="ex_rd" style="font-size:30px;">&nbsp; 1달 이용권 100원</label>
<br>
<input type="radio" id="ex_rd1" style="width:30px; height:30px;"name="ex_rds">
<label for="ex_rd1" style="font-size:30px;">&nbsp; 1년 이용권 200원</label>
</div>
<form  id="priceform" onsubmit="return buy();" action="buy.php" method="post">

<input type="hidden" name="price"id="price" value="dddd" >
<button style="width:300px; font-size:20px; margin-left:400px; margin-top:80px;" onclick="return priceforticket();"  type="submit" name="button">결제하기</button>
</form>
</div>
