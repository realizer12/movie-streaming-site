...
<div id="title-of-noticeboard" >
  ...
  //아래 부분은  로그인 아이디  세션으로  구분 하기위해 필요한 코드이다.
    ...
    $sql1 = "select*from movieblog where idnum='$number'";
    // $sql = "select count(*) from test3 where id='$id'";
    ...
    //아래의 경우 세션아이디가 admin또는 작성자의 아이디와 같지 않을 경우  alert 와 함께  돌아가도록 조치해두었다.
      ...
      if($_COOKIE[$_SESSION[id].$rows1[idnum].$dsdsd]==$_SESSION[id].$rows1[idnum].$dsdsd){
      //이렇게 쿠키 네임이 같을 경우에는  조회수가  올라가는 코드가  작동이 안되도록 하였다.

    }else{
    $query1 = "UPDATE movieblog SET view=view+1 WHERE idnum='$id'";//조회수를  1씩 올려준다.
    $result=mysqli_query($conn,$query1);

    $cookie_name = $_SESSION[id].$rows1[idnum].$dsdsd;//조회수 중복방지 쿠키 이름
    $cookie_value = $_SESSION[id].$rows1[idnum].$dsdsd;
...
영화후기 블로그
  <?php if($_SESSION[id]==$rows4[id]) {?>
<div id="fixboardcontainer" style="margin-top:-80px; margin-bottom:-91px; margin-left:970px; color:white; ">
  <form  action="deletemovieblogcontent.php" method="post">

    <input  type="hidden" id="deletednumber" name="deletednumber" value="<?php echo $_GET[number] ?>">
    <button type="button" style="width:100px; font-size:20px; " onclick="location.href='updatemovieblogcontent.php?page=<?php echo $pageno ?>&number=<?php echo $number; ?>'"name="button">수정</button>
    <button type="submit" onclick="return confrim_delete();" style="width:100px; font-size:20px;" name="button">삭제</button>

    <button type="button" style="width:100px; font-size:20px;"onclick="location.href='movieblog.php?no=<?php echo $pageno; ?>&opt=<?php echo $_GET[opt] ?>&searchfromcontent=<?php echo $_GET[searchfromcontent] ?>'" name="button">목록</button>
  </form>
</div>
<?php }else{ ?>
<div id="fixboardcontainer" style="margin-top:-80px; margin-bottom:-91px; margin-left:1200px; color:white; ">
  <form  action="deletemovieblogcontent.php" method="post">
    <input  type="hidden" id="deletednumber" name="deletednumber" value="<?php echo $_GET[number] ?>">
...
<div id="commentcontainer">
  ...
  $mysql_db = 'testdb';
  $conn = mysqli_connect($mysql_host, $mysql_user, $mysql_password,$mysql_db);
  $query = "SELECT * FROM blogcomment ORDER BY grp ASC";//desc와 반대되는 차순임. 오름 차순임
  ...
  $total_count="SELECT id FROM blogcomment WHERE articlenum = $presentarticlenumber ";//현재 페이지의  댓글의 개수 가 몇개인지알아보기위한 쿼리 문이다.
