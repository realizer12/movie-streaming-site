...
<script type="text/javascript">
  function checkboardbeforeupload() {
          if(document.getElementById('titleofboard').value==""){
            ...
            document.getElementById('summernotecontent').value=$('#summernote').summernote('code');
          }
  }
</script>
<form id="boardform"onsubmit="return checkboardbeforeupload();" action="uploadmovieblog.php" method="post">
<div id="titledivforboard" style="margin-top:-20px;">
  <label for="titleofboard" style="font-size:30px;">글 제목:</label>
  <input type="hidden" id="imgcountcheck" name="imgcountcheck" value="0">
  <input type="text" style="margin-right: 5px;margin-top: 10px;font-size:18px; width:400px; " maxlength="20" placeholder="제목은 20자내외로 적으세요!"   name="titleofboard" id="titleofboard" value="">
</div>
...
<!--실제 게시판 들어감.  -->
  ...
  <textarea id="summernote" form="boardform"style="font-size:20px;"></textarea>
  <button type="submit" id="uploadwrite" style="font-size:20px;"name="uploadwrite">글 올리기</button>
  <div id="contatinercancel">
  <button type="button" id="uploadwritecancle" onclick="clickcancle()"style="font-size:20px; "name="uploadwritecancle">작성 취소</button>
  </div>
...
<script>
      ...
      $(document).ready(function(){
           $('#summernote').summernote({
             ...
             placeholder:'영화 후기를 적어주세요!',
             fontNamesIgnoreCheck: ['Hanna'],
             toolbar:[['style',['fontname','fontsize','color','bold', 'italic', 'underline', 'clear','video','picture','paragraph','height']]],//에디어 툴바 목록
             height : 583, // 에디터의 높이
             width:1296,//에디어 넓이
